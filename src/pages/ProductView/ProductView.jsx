...
export default function ProductView() {
	...
	const getProducts = useCallback(async () => {
		...
		try {
			const res = await Axios.get(
				`/products/?limit=1000&min_price=${
					minPrice && minPrice
				}&max_price=${maxPrice && maxPrice}&categories__in=${
					category.length > 0 ? category.map((item) => item) : select
				}`
			);
	...
	}, [category, minPrice, maxPrice, select]);
