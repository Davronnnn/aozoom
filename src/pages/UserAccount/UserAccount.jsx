...
import { UserAccountWrapper } from './userAccounStyle';
import { useDispatch, useSelector } from 'react-redux';
import {
	getUserOrders,
	getUserInfo,
	editPartnerInfo,
} from '../../Redux/userInfos/user';
	...
	const handleSubmit = async (data, { resetForm }) => {
		...
		try {
			const originalPromiseResult = await dispatch(
				editPartnerInfo({ content: answer, id: userInfo?.id })
			).unwrap();
			dispatch(getUserInfo());
				...
				{role === 'Customer' ? (
					<UserAccountWrapper>
						<div className='left-side'>
							<ul>
								<li onClick={toUserDetail}>
									{' '}
									<img src={userIcon} alt='user' />{' '}
									{t('Личные данные')}
								</li>
								<li onClick={toOrdersSection}>
									<img src={orderIcon} alt='chart' />{' '}
									{t('Заказы')}
								</li>
								<h4 onClick={() => logout()}>
									{' '}
									<img src={exit} alt='exit' /> {t('Выйти')}
								</h4>
							</ul>
						</div>
							...
							{!orderSection ? (
								<>
									...
									<div className='flex-item'>
										<p className='item'>{t('Ф.И.О')}</p>{' '}
										<p className='item'>{userInfo?.name}</p>
									</div>
									...
									<div className='flex-item'>
										<p className='item'>{t('ИНН')}</p>{' '}
										<p className='item'>{userInfo?.inn}</p>
									</div>
					...
					<UserAccountWrapper>
						<div className='left-side'>
							<ul>
								...
								<li onClick={handleViewPurchase}>
									<img src={orderIcon} alt='chart' />
									{t('p7')}
								</li>
								<li onClick={() => set_edit_partnerInfo(false)}>
									<img src={userIcon} alt='user' />{' '}
									{t('Личные данные')}
								</li>
								<h4 onClick={() => logout()}>
						...
						<div className='right-side'>
							{!edit_partnerInfo ? (
								<>
									...
									<div className='flex-item'>
										<p className='item'>{t('МФО')}</p>{' '}
										<p className='item'>{userInfo?.mfo}</p>
									</div>
								...
								<>
									...
									!viewEditProduct ? (
										<Purchases
											handleViewAddProduct={
												handleViewAddProduct
											}
											handleViewEditProduct={
												handleViewEditProduct
											}
										/>
									) : !viewPurchase &&
